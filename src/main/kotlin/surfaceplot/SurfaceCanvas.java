        ...
        while (i <= calc_divisions)
        {
            while (j <= calc_divisions)
            {
                ...
                tmpVertices[k] = new Point3D((x - xi) * xfactor - 10,
                        (y - yi) * yfactor - 10, v);
                tmpVertices[k].unscaledPoint = new Point3D(x, y, v);
                j++;
                y += stepy;
                k++;
            }
...
//	}
    ...
    private void plotPlane(Point3D[] vertex, int verticescount)
    {
        ...
        boolean extra = this.shouldMarkExtraPoints(vertex);
    ...
    private boolean shouldMarkExtraPoints(Point3D[] vertex)
    {
        if (this.model.getExtraVertices().length == 0) return false;

        Point3D original = vertex[0].unscaledPoint;
        Path2D path = new Path2D.Double();
        path.moveTo(original.x, original.y);
        for(int i = 1; i < vertex.length; ++i)
        {
            original = vertex[i].unscaledPoint;
            path.lineTo(original.x, original.y);
        }
        path.closePath();

        return Arrays.stream(this.model.getExtraVertices()).anyMatch(p -> path.contains(p.x, p.y));
    }
